...
<head>
  ...
  <style>
    *,*::before,*::after{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:#f7f5ef;color:#111;font-family:'Noto Sans JP',sans-serif;line-height:1.85;overflow-x:hidden}}a{{color:inherit}}img{{max-width:100%;display:block}}.container{{width:min(1120px,calc(100% - 40px));margin:auto}}
    ...
    .section{{padding:82px 0}}.section.alt{{background:#fff}}.section-head{{display:flex;align-items:end;justify-content:space-between;gap:28px;margin-bottom:34px}}.section-title{{margin:0;font-size:clamp(28px,4vw,46px);line-height:1.25;text-wrap:balance}}.section-lead{{max-width:680px;margin:14px 0 0;color:#555;font-weight:500}}.column-grid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:22px}}.column-card{{background:#fff;border:1px solid #e5e1da;border-radius:18px;overflow:hidden;box-shadow:0 14px 34px rgba(0,0,0,.06);display:flex;flex-direction:column}}.column-card img{{width:100%;aspect-ratio:16/9;object-fit:cover;background:#eef2f5}}.column-card-body{{padding:22px;display:flex;flex-direction:column;gap:10px;flex:1}}.column-card time,.summary-date{{color:#4a90d9;font-size:12px;font-weight:800;letter-spacing:.08em}}.column-card h2{{margin:0;font-size:18px;line-height:1.55;text-wrap:balance}}.column-card p{{margin:0;color:#555;font-size:14px;font-weight:500}}.read-more{{margin-top:auto;color:#1f6db4;font-weight:800;text-decoration:none}}.note-link-box{{margin-top:38px;padding:26px;background:#eef8f2;border:1px solid #bce9c9;border-radius:16px;display:flex;align-items:center;justify-content:space-between;gap:18px}}.note-link-box p{{margin:0;color:#244b33;font-weight:700}}
    ...
    .sticky-cta{{position:fixed;right:18px;bottom:18px;z-index:55;display:flex;gap:10px}}.sticky-cta a{{display:flex;align-items:center;justify-content:center;min-height:46px;padding:12px 16px;border-radius:6px;text-decoration:none;font-size:12px;font-weight:800;box-shadow:0 12px 24px rgba(0,0,0,.16)}}.sticky-cta .line{{background:#06C755;color:#fff}}.sticky-cta .form{{background:#111;color:#fff}}
    @media(max-width:900px){{.nav{{display:none}}.hamburger{{display:block}}.column-grid{{grid-template-columns:repeat(2,minmax(0,1fr))}}.section-head{{display:block}}.footer-inner{{display:block}}.footer-meta{{text-align:left;margin-top:24px}}}}
    @media(max-width:640px){{.container{{width:calc(100% - 32px)}}.head{{height:68px}}.hero{{padding:64px 0 44px}}.hero-actions{{display:grid;grid-template-columns:1fr}}.btn{{width:100%}}.section{{padding:62px 0}}.column-grid{{grid-template-columns:1fr;gap:16px}}.column-card-body{{padding:18px}}.note-link-box{{display:block}}.note-link-box .btn{{margin-top:16px}}.sticky-cta{{left:0;right:0;bottom:0;gap:0}}.sticky-cta a{{flex:1;border-radius:0;min-height:54px}}body{{padding-bottom:54px}}}}
  </style>
</head>
<body>
{body}
    ...
    <section class="section">
      <div class="container">
        <div class="section-head">
          ...
          <p class="section-lead">検索・AI検索で見つけてもらう入口として、noteの内容を自社サイト上でも整理しています。</p>
        </div>
